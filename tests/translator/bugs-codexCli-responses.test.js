...
describe("Codex CLI Responses → OpenAI", () => {
  it("rejects a nameless function_call instead of emitting empty tool_calls", () => {
    expect(() => R2O({
  ...
  it("rejects unresolved input_image.file_id instead of treating it as a URL", () => {
    expect(() => R2O({
      input: [{ type: "message", role: "user", content: [{ type: "input_image", file_id: "file-abc" }] }],
    })).toThrow("file_id image");
  });
});
